				...
				if (workbook.getSheetName(i).equalsIgnoreCase("APIData")) {
					...
					while (row.hasNext()) {
						...
						while (cell.hasNext()) {
							...
							if (value.getStringCellValue().equalsIgnoreCase("TestCase")) // Conditioning for the Data2
																							// existence cell value
							{
								// desired Cell
								column = k;
							}
						...
						System.out.println("Column number of TestCase is " + column);
						System.out.println("Inside 2nd Row While Loop");
						while (row.hasNext()) { // Once column is got iterate for the that row and it's cell value
							Row r = row.next();

							if (r.getCell(column).getStringCellValue().equalsIgnoreCase(TestCase)) { // condition
																										// testcase name
																										// row found
								...
								while (c2.hasNext()) { // checking cell for value
									...
									Cell c = c2.next(); //
									if (c.getCellTypeEnum().equals(CellType.STRING)) // checking if cell have string
																						// value
									{
